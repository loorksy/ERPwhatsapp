...
function App() {
    ...
    () => [
      {
        ...
        helper: health ? `Environment: ${health.environment}` : 'Waiting for first ping...',
      },
      {
        label: 'WhatsApp',
        value: 'Pending',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: 'Message queued successfully' });
      setForm({ phone: '', message: '' });
    } catch (error) {
      setToast({ type: 'error', message: error?.response?.data?.message || 'Failed to queue message' });
    } finally {
      setSending(false);
    ...
    <div className="min-h-screen px-6 py-10">
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-900">لوحة التحكم للنظام الذكي</h1>
          <p className="mt-1 text-sm text-slate-600">إدارة الإعدادات ومراقبة الاتصال وإرسال الرسائل التجريبية.</p>
        </div>
